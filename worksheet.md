...
Your team want to expand the type of actors that can appear in the game to include "impediments". These are actors that are not on team red or team blue, they just take up a space on the board and get in the way of the other players.  You have been given a library of existing impediments to work with in a jar file `impediments.jar`.  Someone has updated the project with this jar file and changed the classpath to access it but it is now your job to add these impediments to your project and have them act as actors in the game.

NB:  The person who update the project also added an `isTeamBlue` method so you don't have to keep using `!isTeamRed()`.  This is needed because the impediments will _be_ actors but will not be on team red or team blue (i.e. both methods need to return false for impediments).

To help out, the signatures and some documentation of all the classes and methods in the impediments jar file are given here:

~~~~~
/**
 * An impediment is something in a game that "gets in the way".  They have a representation on screen and they 
 * can be moved.  However, they make their own mind up about how they will move.
 * 
 * @param <X> the Type of the object that will contain this impediment.  Must extend java.awt.Rectangle so this impediment
 * can know the borders of its space and draw itself
 */
public class Impediment<X extends Rectangle> {

    /**
     * Create an impediment that is within object X on the screen.
     * @param location
     */
    public Impediment(X location);

    /**
     * Update the display of this impediment in case the containing object has changed.
     */
    public void setPoly();

    /**
     * Ask this impediment to move.  It might not move, and if it does, it might take a while.
     * @param newLocation The location to move to.
     */
    public void moveAtOwnSpeed(X newLocation);

    /**
     * Put the representation of this impediment onto the given canvas
     * @param g Canvas to draw on.
     */
    public void paint(Graphics g);

    /**
     * Find out what object (it must extend Rectangle) contains this impediment.
     * @return
     */
...
~~~~~
/**
 * A mole is an impediment that can move, but it will only do so 1% of the time and it will take 2 seconds to do so.
 */
public class Mole<X extends Rectangle> extends Impediment<X>
~~~~~

~~~~~
/**
 * A butterfly is an impediment that will move as soon as you ask it to.
 */
public class Butterfly<X extends Rectangle> extends Impediment<X>
~~~~~

~~~~~
/**
 * A wall is an impediment that never moves, no matter how often you ask it to.
 */
public class Wall<X extends Rectangle> extends Impediment<X>
~~~~~

Your task is to choose the right pattern to help you incorporate these impediments _as actors_ in the game.  Once you have done that, implmenent the pattern and then check it is working by doing something with these impediments.